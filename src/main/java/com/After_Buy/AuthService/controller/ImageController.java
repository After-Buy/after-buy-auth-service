...
import com.After_Buy.AuthService.service.S3Service;
...
public class ImageController {
     ...
     */
    @Operation(summary="프로필 이미지 Pre-signed URL 발급", description="S3 직접 업로드용 Pre-signed URL을 발급합니다. 발급된 URL로 PUT 요청하여 이미지를 업로드하세요.", security={@SecurityRequirement(name="bearerAuth")})
    ...
    public ResponseEntity<ApiResponse<Map<String, Object>>> getPresignedUrl(@AuthenticationPrincipal UserPrincipal principal, @Valid @RequestBody PresignedUrlRequest request) {
        Map<String, Object> data = s3Service.generatePresignedUrl(principal.getUserId(), request.getFileExtension());
