...
/**
 * JWT 토큰 검증 필터
 * 스프링 시큐리티의 인증 필터 체인 앞단에서 매 요청마다 넘어오는 헤더의 토큰을 파싱하고 시큐리티 컨텍스트에 인증 객체를 심어주는 역할을 합니다.
 *
 * @since : 2026.04.04
 * @version : 1.0.0
 * @author : 신태훈
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    /**
     * 필터 체인 내부 검증 로직 구현부
     * 서블릿 컨테이너로 들어오는 모든 요청에 대해 Authorization 헤더를 뜯어보고 정상적인 토큰일 시 UserPrincipal을 주입합니다.
     *
     * @param request : 클라이언트가 보낸 HTTP 서블릿 리퀘스트
     * @param response : 클라이언트에게 보낼 HTTP 서블릿 리스폰스
     * @param filterChain : 뒤에 이어질 보안 및 컨트롤러 체인 라인
     * @throws ServletException : 서블릿 단 예외 발생 시 필터에서 처리 불가 에러
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (StringUtils.hasText(token) && jwtTokenProvider.validateToken(token)) {
            try {
                ...
                UsernamePasswordAuthenticationToken authentication = new UsernamePasswordAuthenticationToken(userPrincipal, null, List.of());
